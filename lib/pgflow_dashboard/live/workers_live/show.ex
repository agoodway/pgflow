defmodule PgFlowDashboard.Live.WorkersLive.Show do
  ...
  defp navigate_to_adjacent_worker(socket, direction) do
    case Workers.adjacent(socket.assigns.repo, socket.assigns.worker_id, direction) do
  ...
  defp load_worker(socket) do
    case Workers.get(socket.assigns.repo, socket.assigns.worker_id) do
  ...
  defp load_worker_tasks(socket) do
    if socket.assigns.worker do
      {:ok, tasks} = Workers.list_tasks(socket.assigns.repo, socket.assigns.worker_id, limit: 50)
