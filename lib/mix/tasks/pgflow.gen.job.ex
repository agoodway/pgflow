defmodule Mix.Tasks.Pgflow.Gen.Job do
  ...
  that creates the flow, adds the `:perform` step, and marks it as `flow_type = 'job'`.

  ## Usage

      mix pgflow.gen.job MyApp.Jobs.SendEmail
      mix pgflow.gen.job MyApp.Jobs.SendEmail --migrations-path priv/repo/migrations
  ...
  Example generated migration:
      ...
      defmodule MyApp.Repo.Migrations.CompileSendEmail do
        ...
        def up do
          ...
          execute "SELECT pgflow.add_step('send_email', 'perform', ARRAY[]::text[], NULL, NULL, NULL, NULL, 'single')"
  ...
  def run(args) do
    ...
    case args do
      ...
      [] ->
        ...
        Usage: mix pgflow.gen.job MyApp.Jobs.MyJob
        """)
  ...
  defp generate_migration(module_string, opts) do
    ...
    unless definition.flow_type == :job do
      ...
      Use `mix pgflow.gen.flow` for flow modules, or make sure this module uses `PgFlow.Job`.
      """)
    end
  ...
  defp generate_migration_content(migration_module, job_slug, sql_statements, has_cron) do
    ...
    defmodule PgFlow.Repo.Migrations.#{migration_module} do
      ...
      Generated by: mix pgflow.gen.job
      \"\"\"
      use Ecto.Migration
