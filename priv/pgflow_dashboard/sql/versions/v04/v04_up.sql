-- Workers persist lower(flow_slug), while the flow retains its logical case.
CREATE OR REPLACE VIEW $SCHEMA$.workers_with_load AS
SELECT
  w.worker_id,
  COALESCE(f.flow_slug, w.queue_name) AS flow_slug,
...
LEFT JOIN pgflow.flows f ON lower(f.flow_slug) = w.queue_name
LEFT JOIN LATERAL (
  SELECT
