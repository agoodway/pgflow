...
BEGIN
  ...
  IF attribute_names = ARRAY['flow_slug', 'run_id', 'step_slug', 'input', 'msg_id', 'task_index', 'flow_input']::text[]
     AND attribute_types = ARRAY['text', 'uuid', 'text', 'jsonb', 'bigint', 'integer', 'jsonb']::text[] THEN
...
$$
$pgflow_seven$;
  ELSIF attribute_names = ARRAY['flow_slug', 'run_id', 'step_slug', 'input', 'msg_id', 'task_index', 'flow_input', 'attempts_count']::text[]
        AND attribute_types = ARRAY['text', 'uuid', 'text', 'jsonb', 'bigint', 'integer', 'jsonb', 'integer']::text[] THEN
...
CREATE FUNCTION "pgflow"."start_tasks" ("flow_slug" text, "msg_ids" bigint[], "worker_id" uuid, "queue_name" text) RETURNS SETOF "pgflow"."step_task_record" LANGUAGE sql SET "search_path" = '' AS $$
with task_candidates as (
    select
      task.flow_slug,
      task.run_id,
