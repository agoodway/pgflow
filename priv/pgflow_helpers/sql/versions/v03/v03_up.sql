-- WRITE: `start_tasks` returns the task's attempt number.
--
-- `PgFlow.Context` documents `:attempt` as "Current attempt number
-- (1-indexed)", but the worker had no way to populate it: `start_tasks`
-- increments `step_tasks.attempts_count` before dispatching and then does not
-- return it, so every handler saw a hardcoded 1 however many times it had been
-- retried. A handler that must act only on its final attempt — marking a record
-- failed rather than leaving it mid-flight — could not tell.
--
-- Two changes, both additive:
--
--   * `pgflow.step_task_record` gains an `attempts_count` attribute. Upstream
--     extended this same type once before (`task_index`), so the shape is not
--     treated as frozen.
--   * `pgflow.start_tasks` is replaced with the upstream body plus that column.
--
-- `poll_for_tasks` also returns this type but is deprecated upstream: its body
-- raises a notice and returns no rows, so a wider record cannot affect it and
-- it is deliberately left alone.
--
-- Because this replaces an upstream function, a future `mix pgflow.sync_upstream`
-- that changes `start_tasks` must be reconciled here by hand — the same standing
-- cost v02 accepted for `recover_stalled_tasks`.

ALTER TYPE $SCHEMA$.step_task_record ADD ATTRIBUTE attempts_count integer;

--SPLIT--

CREATE OR REPLACE FUNCTION $SCHEMA$.start_tasks(flow_slug text, msg_ids bigint[], worker_id uuid)
 RETURNS SETOF $SCHEMA$.step_task_record
 LANGUAGE sql
 SET search_path TO ''
AS $function$
with tasks as (
  ...
  start_tasks_update as (
    ...
    set
      -- Qualified: the `tasks` CTE now also exposes attempts_count, so a bare
      -- reference here is ambiguous.
      attempts_count = step_tasks.attempts_count + 1,
      status = 'started',
      started_at = now(),
  ...
  select
    ...
    CASE
      ...
      ELSE NULL
    END as flow_input,
    -- The attempt this dispatch represents. `tasks` is materialized before the
    -- `start_tasks_update` CTE runs, and Postgres does not expose a data-modifying
    -- CTE's effects to the rest of the same statement, so this reads the count as
    -- it was BEFORE the increment above. Adding one yields the attempt the handler
    -- is actually on, 1-indexed to match `PgFlow.Context`.
    st.attempts_count + 1 as attempts_count
  from tasks st
