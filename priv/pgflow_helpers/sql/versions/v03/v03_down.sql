-- Roll back to the pre-v03 `start_tasks` body (no attempts_count column), then
-- narrow the composite type again.
--
-- Order matters: the function must stop selecting the attribute before the
-- attribute is dropped, or the drop fails on a dependent function.
...
ALTER TYPE $SCHEMA$.step_task_record DROP ATTRIBUTE attempts_count;
